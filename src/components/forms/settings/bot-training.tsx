...
import { HELP_DESK_TABS_MENU} from "@/constant/menu";
import React from "react";
import HelpDesk from "./help-desk";
import FilteredQuestions from "./filtered-questions";
...
const BotTrainingForm = ({ id }: Props) => {
  return (
    <div className="py-5 mb-10 flex flex-col gap-5 items-start">
      ...
      <TabsMenu triggers={HELP_DESK_TABS_MENU} className="">
        <TabsContent value="help desk" className="w-full">
          <HelpDesk id={id} />
        </TabsContent>
        <TabsContent value="questions">
          <FilteredQuestions id={id} />
        </TabsContent>
      </TabsMenu>
    </div>
  );
